...
static const point_t map_center_point =
{ .x = ILI9488_HEIGHT / 2, .y = ILI9488_WIDTH / 2, .color = WHITE };
static const point_t map_invalid_point =
{ 0 };
...
static point_t map_point_buf[POINT_BUF_SIZE] =
{ 0 };
...
static point_t map_selected_point[2] =
{ map_invalid_point, map_invalid_point };
...
void MAP_DrawSamples(void)
{
	...
	{
		point_t new_point;
		bool is_valid = _ConvertSampleToPoint(
				&map_sample_buf[map_sample_read_idx], &new_point);
		...
		if (is_valid)
		{
			...
			if (point->x != 0 || point->y != 0)
			{
				...
				case MAP_PERSIST_ONESHOT:
					...
					if (HAL_GetTick()
							- map_persistence_start_tick> MAP_PERSISTENCE_ONESHOT_DURATION /*ms*/)
					{
...
void MAP_HandleTouch(void)
{
	...
	if (XPT2046_GotATouch() && XPT2046_GetTouchPosition(&x, &y))
	{
		...
		if (x >= MAP_BUTTON_START_X
				&& x < MAP_BUTTON_START_X + MAP_BUTTON_START_W
				&& y >= MAP_BUTTON_START_Y
		...
		else if (x >= MAP_BUTTON_SCALE_X
				&& x < MAP_BUTTON_SCALE_X + MAP_BUTTON_SCALE_W
				&& y >= MAP_BUTTON_SCALE_Y
		...
		else if (x >= MAP_BUTTON_QUAL_MINUS_X
				&& x < MAP_BUTTON_QUAL_MINUS_X + MAP_BUTTON_QUAL_MINUS_W
				&& y >= MAP_BUTTON_QUAL_MINUS_Y
				&& y < MAP_BUTTON_QUAL_MINUS_Y + MAP_BUTTON_QUAL_MINUS_H)
		{
			...
			uint8_t new_quality =
					map_quality_min < 6 ? 0 : (map_quality_min - 6);

			MAP_SetQuality(new_quality);
			_DrawQualityMinimum(map_quality_min);
			MAP_ClearPoints(true);
		}
		else if (x >= MAP_BUTTON_QUAL_PLUS_X
				&& x < MAP_BUTTON_QUAL_PLUS_X + MAP_BUTTON_QUAL_PLUS_W
				&& y >= MAP_BUTTON_QUAL_PLUS_Y
				&& y < MAP_BUTTON_QUAL_PLUS_Y + MAP_BUTTON_QUAL_PLUS_H)
		{
			// Button QUALITY + pressed
			static uint32_t tick_pressed = 0;
			if (tick_cur - tick_pressed < MAP_BUTTON_DEBOUNCE_TIMER)
			...
			uint8_t new_quality =
					map_quality_min > 57 ? 63 : (map_quality_min + 6);

			MAP_SetQuality(new_quality);
			_DrawQualityMinimum(map_quality_min);
			MAP_ClearPoints(true);
		}
		else if (x >= MAP_BUTTON_PERS_MODE_X
				&& x < MAP_BUTTON_PERS_MODE_X + MAP_BUTTON_PERS_MODE_W
				&& y >= MAP_BUTTON_PERS_MODE_Y
				&& y < MAP_BUTTON_PERS_MODE_Y + MAP_BUTTON_PERS_MODE_H)
		{
			...
			map_persistence_mode_e new_mode = (map_persistence_mode + 1)
					% MAP_PERSIST_MAX;
			MAP_SetPersistanceMode(new_mode);
			_DrawPersistanceButtons(map_persistence_mode);
			MAP_ClearPoints(false);
		}
		else if (x >= MAP_BUTTON_PERS_CLEAR_X
				&& x < MAP_BUTTON_PERS_CLEAR_X + MAP_BUTTON_PERS_CLEAR_W
				&& y >= MAP_BUTTON_PERS_CLEAR_Y
				&& y < MAP_BUTTON_PERS_CLEAR_Y + MAP_BUTTON_PERS_CLEAR_H)
		{
		...
		else if (x >= MAP_TOOLBAR_WIDTH && x <= (MAP_TOOLBAR_WIDTH + MAP_SIZE))
		{
			...
			if (map_selected_point[map_selected_point_idx].x != 0)
			{
				...
				ILI9488_FillCircle(map_center_point.x, map_center_point.y, 3,
						map_center_point.color);
			}

			map_selected_point[map_selected_point_idx].x = x;
			map_selected_point[map_selected_point_idx].y = y;
			map_selected_point[map_selected_point_idx].color =
					map_selected_point_idx == 0 ? BLUE : CYAN;
			ILI9488_FillCircle(x, y, 3,
					map_selected_point[map_selected_point_idx].color);
			_DrawDistanceInfo(&map_selected_point[0], &map_selected_point[1]);
...
void MAP_ClearPoints(bool erase_buffers)
{
	...
	if (erase_buffers)
	{
		...
		memset(&map_sample_buf[0], 0,
				sizeof(rplidar_measurement_t) * SAMPLE_BUF_SIZE);
...
static bool _ConvertSampleToPoint(rplidar_measurement_t *sample, point_t *point)
{
	...
	point->color = color565(0xFF - (sample->quality * 4), sample->quality * 4,
			0x00); // Quality range:  0-63

	return (point->x >= MAP_TOOLBAR_WIDTH)
			&& (point->x <= (MAP_TOOLBAR_WIDTH + MAP_SIZE))
			&& (point->y <= MAP_SIZE);
}
...
static void _DrawGrid(void)
{
	...
	for (uint8_t i = 0; i < 6; i++)
	{
		...
		ILI9488_FillArea(MAP_TOOLBAR_WIDTH,
				i != 5 ? (MAP_SIZE / 5) * i : MAP_SIZE - 1, MAP_SIZE, 1,
				DDDD_WHITE);
...
static void _DrawMapScale(double scale)
{
	...
	ILI9488_CString(0, 0, MAP_TOOLBAR_WIDTH - 16, MAP_SIZE / 5, scale_txt,
			Font12, 1, WHITE, BLACK);
}
...
static void _DrawButtonStart(bool is_started)
{
	...
	ILI9488_WIDTH - 10, is_started ? "STOP" : "START", Font16, 1, WHITE,
			is_started ? RED : DD_GREEN);
	ILI9488_DrawBorder(MAP_BUTTON_START_X, MAP_BUTTON_START_Y,
	MAP_BUTTON_START_W, MAP_BUTTON_START_H, 2,
	WHITE);
}
...
static void _DrawButtonScale(map_scale_mode_e mode)
{
	...
	MAP_BUTTON_SCALE_W, MAP_BUTTON_SCALE_H, 2,
	WHITE);
}
...
static void _DrawQualityGradient(void)
{
	for (uint8_t i = 0; i < MAP_QUALITY_GRADIENT_NB; i++)
	{
		ILI9488_FillArea(MAP_QUALITY_GRADIENT_X,
				i
						* (MAP_QUALITY_GRADIENT_H / MAP_QUALITY_GRADIENT_NB)+ MAP_QUALITY_GRADIENT_Y,
				MAP_QUALITY_GRADIENT_W,
				MAP_QUALITY_GRADIENT_H / MAP_QUALITY_GRADIENT_NB,
				color565(i * 25, 0xFF - i * 25, 0x00));
	}
	ILI9488_FillArea(MAP_QUALITY_GRADIENT_X + 10, MAP_QUALITY_GRADIENT_Y, 5, 2,
	WHITE);
	ILI9488_FillArea(MAP_QUALITY_GRADIENT_X + 10,
	MAP_QUALITY_GRADIENT_Y + MAP_QUALITY_GRADIENT_H - 1, 5, 2, WHITE);
	ILI9488_WString(MAP_QUALITY_GRADIENT_X + 17, MAP_QUALITY_GRADIENT_Y - 5,
			"100", Font12, 1, WHITE, BLACK);
	ILI9488_WString(MAP_QUALITY_GRADIENT_X + 17,
	MAP_QUALITY_GRADIENT_Y + MAP_QUALITY_GRADIENT_H - 5, "0", Font12, 1,
	WHITE, BLACK);
	ILI9488_Orientation(ILI9488_Orientation_180);
	ILI9488_CString(10, 5, MAP_QUALITY_GRADIENT_H + MAP_QUALITY_GRADIENT_Y + 5,
			10, "QUALITY(%)", Font16, 1, WHITE, BLACK);
...
{
	char quality_txt[5];
	snprintf(quality_txt, sizeof(quality_txt), "%hu",
			(uint8_t) (round((quality / 63.0) * 10)) * 10);

	ILI9488_CString(ILI9488_HEIGHT - MAP_TOOLBAR_WIDTH + 1, 130, ILI9488_HEIGHT,
			130, "MIN", Font16, 1, WHITE, BLACK);
	ILI9488_CString(ILI9488_HEIGHT - MAP_TOOLBAR_WIDTH + 1, 155, ILI9488_HEIGHT,
			175, quality_txt, Font16, 1, WHITE, BLACK);
	// Sign -
	ILI9488_DrawBorder(MAP_BUTTON_QUAL_MINUS_X, MAP_BUTTON_QUAL_MINUS_Y,
	MAP_BUTTON_QUAL_MINUS_W, MAP_BUTTON_QUAL_MINUS_H, 2, WHITE);
	ILI9488_FillArea(MAP_BUTTON_QUAL_MINUS_X + 5, MAP_BUTTON_QUAL_MINUS_Y + 9,
	MAP_BUTTON_QUAL_MINUS_W / 2, 2, WHITE);
	// Sign +
	ILI9488_DrawBorder(MAP_BUTTON_QUAL_PLUS_X, MAP_BUTTON_QUAL_PLUS_Y,
	MAP_BUTTON_QUAL_PLUS_W, MAP_BUTTON_QUAL_PLUS_H, 2, WHITE);
...
static void _DrawPersistanceButtons(map_persistence_mode_e mode)
{
	...
	ILI9488_CString(ILI9488_HEIGHT - MAP_TOOLBAR_WIDTH + 1, 190, ILI9488_HEIGHT,
			190, "PERSIS", Font16, 1, WHITE, BLACK);
	...
	MAP_BUTTON_PERS_MODE_W, MAP_BUTTON_PERS_MODE_H, 2, WHITE);

	ILI9488_CString(MAP_BUTTON_PERS_CLEAR_X, MAP_BUTTON_PERS_CLEAR_Y,
	ILI9488_HEIGHT - 4, ILI9488_WIDTH - 11, "CLEAR", Font16, 1, WHITE,
	ORANGE);
	ILI9488_DrawBorder(MAP_BUTTON_PERS_CLEAR_X, MAP_BUTTON_PERS_CLEAR_Y,
	MAP_BUTTON_PERS_CLEAR_W, MAP_BUTTON_PERS_CLEAR_H, 2, WHITE);
}
...
static void _DrawDistanceInfo(const point_t *p1, const point_t *p2)
{
	...
	if (p1->x != map_invalid_point.x)
	{
		...
		snprintf(dist_mm, sizeof(dist_mm), "%lu mm",
				(uint32_t) _GetDistancePoints(&map_center_point, p1));
		...
		if (p2->x != map_invalid_point.x)
		{
			...
			snprintf(dist_mm, sizeof(dist_mm), "%lu mm",
					(uint32_t) _GetDistancePoints(&map_center_point, p2));
			...
			snprintf(dist_mm, sizeof(dist_mm), "%lu mm",
					(uint32_t) _GetDistancePoints(p1, p2));
